...
(function( $, undefined ) {
  ...
  $.widget( "ui.addresspicker", {
    ...
    _create: function() {
      ...
      this.target_field = $(this.options.elements.target_field);
      this.calcRoute = $(this.options.elements.calcRoute);
    ...
    _updatePosition: function(location) {
      ...
      if(this.target_field && 1 ==0) {
		this.target_field.val(location.lat()+","+location.lng());
		//for( ijk in map ) alert(ijk + " "+map[ijk]);
		//this._initMap();
    ...
    _findInfo: function(result, type) {
      ...
      for (var i = 0; i < result.address_components.length; i++) {
        ...
        if (component.types.indexOf(type) !=-1) {
			if( component.types.indexOf('administrative_area_level_1') != -1 ) return component.long_name.replace('Gouvernorat de ','');
